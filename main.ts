		...
		try {
			...
			try {
				...
				if (pgliteResources) {
					...
					logger.warn(
						'PGlite resources unavailable, using memory mode (existing IDB data will NOT load)'
					);
					this.persistenceManager = new PersistenceManager();
				}
				await this.persistenceManager.initialize();
				logger.info(
					'PersistenceManager initialized (persistent=' +
						this.persistenceManager.isPersistent() +
						')'
				);
			} catch (error) {
			...
			this.sessionService = new SessionService(
				this.persistenceManager.getSessionRepository()
			);
