...
export interface StreamDeltaResult { delta?: string; done?: boolean }
...
export function extractDelta(ev: RunStreamEvent): string | null {
  ...
  if (ev.type === 'raw_model_stream_event') {
    ...
    if (d.event && typeof d.event.delta === 'string') {
      ...
      if (t === 'response.output_text.delta' || t === 'output_text.delta' || t === 'output_text_delta') return d.event.delta as string;
    }
  }
  return null;
}

export async function accumulateStream(asyncIter: AsyncIterable<RunStreamEvent>, onDelta: (text: string) => void): Promise<{ final: string }>{
  let acc = '';
