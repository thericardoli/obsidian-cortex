...
export interface ToolDiagnostic { level: 'warn' | 'error'; message: string; toolName?: string }

export async function buildTools(agentConfig: AgentConfig, ctx: { agentManager: AgentManager }): Promise<{ tools: Tool[]; diagnostics: ToolDiagnostic[] }> {
  ...
  for (const toolConfig of agentConfig.tools) {
    ...
    try {
      ...
      diagnostics.push({ level: 'warn', message: (e instanceof Error ? e.message : 'Unknown tool error'), toolName: toolConfig.name });
    }
  }

  if (diagnostics.length) {
    for (const d of diagnostics) {
      if (d.level === 'warn') logger.warn(`Tool diagnostic: ${d.toolName ?? ''} ${d.message}`);
...
async function buildSingle(toolConfig: ToolConfig, agentManager: AgentManager): Promise<Tool | null> {
  switch (toolConfig.type) {
    case 'function': {
      const executorName = toolConfig.executor;
      const executor = executorName ? functionToolRegistry.get(executorName) : undefined;
      if (!executor) throw new Error(`No executor found for function tool: ${toolConfig.name}`);
    ...
    case 'hosted': {
      const hosted = createHostedTool((toolConfig.name as unknown) as import('./hosted-registry').HostedToolName, toolConfig.providerData ?? {});
      return hosted ?? null;
    }
    case 'agent': {
